...
Map::Map(MapLoader* mapLoaderObject)
{
    ...
    for (int i = 0; i < territoryCount; i++) {
        ...
        terrVector.at(i).armyCount = 0;
        terrVector.at(i).territoryName = mapLoaderObject->countryNameVector.at(i);
        terrVector.at(i).borders.resize(mapLoaderObject->borders.at(i).size());
        for(int j = 0; j < mapLoaderObject->borders.at(i).size(); j++) {
            terrVector.at(i).borders.at(j) = mapLoaderObject->borders.at(i).at(j);
        }
...
Map& Map::operator=(const Map &map) {
    ...
    for (int i = 0; i < territoryCount; i++) {
        ...
        this->terrVector.at(i).armyCount = map.terrVector.at(i).armyCount;
...
//                        borders[i].reserve(line.length());
                        ...
                        for (int j = 0; j < line.length(); j++) {
                            if (line[j] == ' ' || j == line.length() - 1) {
                                ...
                                if(borderToAdd != i+1) {
//                                    borders.at(i).resize(borders.at(i).size() + 1);
                                    borders.at(i).push_back(borderToAdd);
                                }
                                borderString.clear();
                            } else {
                                borderString += line[j];
